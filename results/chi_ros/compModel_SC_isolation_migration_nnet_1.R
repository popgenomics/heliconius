...
target = read.table("/scratch/ul/monthly/croux/ABCh/simulations/largePrior_v4/ABCmonolocus/analysis/summary_stats_lengthScaled_1.txt", h=T)[, ss]
...
res_isolation_migration = model_selection_abc_nnet(target=target, x=x, sumstat=rbind(isolation, migration), tol=1000/(2*nrow(isolation)), noweight=F, rejmethod=F, nb.nnet=10, size.nnet=8, output="OBS_isolation_migration_1.txt")
