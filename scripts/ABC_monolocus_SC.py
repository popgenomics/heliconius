...
# M1: migration in species 1 (M1!=0; M2=0)
os.system("mkdir M1")

spinput = "\n{0}\n{1}\n{2}\n{3}\n{4}\nmyfifo".format(1, nsamA, nsamB, L, nRep)

for sub_rep in range(10):
	os.system("mkdir M1/rep_{0}".format(sub_rep))
	outfile = open("M1/rep_{0}/spinput.txt".format(sub_rep), "w")
	outfile.write(spinput)
	outfile.close()
	
	outfile = open("M1/rep_{0}/prior.txt".format(sub_rep), "w")
	...
	for i in range(nRep):
		...
		M2 = 0 #posterior[12][setOfParam] * beta(posterior[15][setOfParam], posterior[16][setOfParam])
...
# M2: migration in species 2 (M2!=0; M1=0)
os.system("mkdir M2")

spinput = "\n{0}\n{1}\n{2}\n{3}\n{4}\nmyfifo".format(1, nsamA, nsamB, L, nRep)

for sub_rep in range(10):
	os.system("mkdir M2/rep_{0}".format(sub_rep))
	outfile = open("M2/rep_{0}/spinput.txt".format(sub_rep), "w")
	outfile.write(spinput)
	outfile.close()
	
	outfile = open("M2/rep_{0}/prior.txt".format(sub_rep), "w")
	nPosterior = len(posterior[0])
	for i in range(nRep):
		setOfParam = choice(nPosterior)
		N1 = posterior[0][setOfParam] * beta(posterior[9][setOfParam], posterior[10][setOfParam])
		...
		M1 = 0 # posterior[11][setOfParam] * beta(posterior[13][setOfParam], posterior[14][setOfParam])
...
# M3: migration in both directions, asymetric
os.system("mkdir M3")

spinput = "\n{0}\n{1}\n{2}\n{3}\n{4}\nmyfifo".format(1, nsamA, nsamB, L, nRep)

for sub_rep in range(10):
	os.system("mkdir M3/rep_{0}".format(sub_rep))
	outfile = open("M3/rep_{0}/spinput.txt".format(sub_rep), "w")
	outfile.write(spinput)
	outfile.close()
	
	outfile = open("M3/rep_{0}/prior.txt".format(sub_rep), "w")
	nPosterior = len(posterior[0])
	for i in range(nRep):
		setOfParam = choice(nPosterior)
		N1 = posterior[0][setOfParam] * beta(posterior[9][setOfParam], posterior[10][setOfParam])
...
os.system("mkdir M0")
for sub_rep in range(10):
	os.system("mkdir M0/rep_{0}".format(sub_rep))
	outfile = open("M0/rep_{0}/spinput.txt".format(sub_rep), "w")
	outfile.write(spinput)
	outfile.close()
	
	outfile = open("M0/rep_{0}/prior.txt".format(sub_rep), "w")
	nPosterior = len(posterior[0])
	for i in range(nRep):
		setOfParam = choice(nPosterior)
		N1 = posterior[0][setOfParam] * beta(posterior[9][setOfParam], posterior[10][setOfParam])
...
print('bsub -q normal <<<"mknod myfifo p; mscalc < myfifo & msnsam tbs {0} -t tbs -r tbs tbs -I 2 tbs tbs 0 -m 1 2 tbs -m 2 1 tbs -n 1 tbs -en tbs 1 tbs -n 2 tbs -en tbs 2 tbs -eM tbs 0 -ej tbs 2 1 -eN tbs tbs <prior.txt >myfifo"'.format(nRep))
